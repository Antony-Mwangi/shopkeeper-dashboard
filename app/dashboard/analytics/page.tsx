...
export default function AnalyticsPage() {
  ...
  const revenueChart = Object.entries(data.revenueByDate).map(([d, v]) => ({ date: d, revenue: v }));
  const productChart = Object.entries(data.productStats).map(([n, q]) => ({ name: n, quantity: q }));
  ...
  return (
    <div className="space-y-6">
          ...
          {["today", "week", "month"].map(r => (
            <button
              ...
              className={`px-4 py-2 rounded ${
                range === r ? "bg-black text-white" : "bg-white border"
              }`}
            >
              {r}
            </button>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Card title="Revenue" value={`Ksh ${data.totalRevenue}`} />
        <Card title="Sales" value={data.totalSales} />
        <Card title="Trend" value={`${data.revenueTrend.toFixed(1)}%`} />
      </div>
